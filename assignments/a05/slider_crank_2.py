...
while (application.GetDevice().run()):
    ...
    time_array.append(system.GetChTime())
    slider_pos.append(slider.GetPos().x)  
    motor_tq.append(engine_ground_crank.GetMotorTorque())
    ...
    if system.GetChTime() > 4:
      application.GetDevice().closeDevice()


plt.figure()
plt.plot(time_array,slider_pos)
plt.xlabel('time (s)')
plt.ylabel('slider position (m)')
plt.title('slider position over time')
plt.show()
plt.figure()
plt.plot(time_array[10:-1],motor_tq[10:-1])
plt.xlabel('time (s)')
